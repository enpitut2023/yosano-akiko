  ...
  if (tableYear === 2023) {
    switch (specialty) {
      case "med":
        ...
        if (b["医療概論I"] || b["医療概論I-B"]) return "a3";
        if (b["医学の基礎"] || b["医学の基礎B"] || b["医科分子生物学"])
          return "a4";
...
function classify(
  ...
  // 必修
  const a = classifyColumnA(id, tableYear, specialty);
...
export function classifyKnownCourses(
  ...
  for (const c of cs) {
    const cellId = classify(c.id, c.name, "known", tableYear, specialty);
    if (cellId !== undefined) {
...
export function classifyRealCourses(
  ...
  const courseIdToCellId = new Map<CourseId, string>();
  for (const c of cs) {
    const cellId = classify(c.id, c.name, "real", tableYear, specialty);
    if (cellId !== undefined) {
      courseIdToCellId.set(c.id, cellId);
    }
  }
